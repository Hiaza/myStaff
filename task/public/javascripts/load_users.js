...
Promise.all([
    fetch("/templates/users_list.mst").then(x => x.text()),
    fetch("/api/v1/users?page="+page+"&search="+search).then(x => x.json()),
])
    ...
    .then((result) => {
        ...
        if(parseInt(items.pageCount)==items.curPage+1||items.pageCount==1){
            document.getElementById('next_users').disabled = true;
        }else document.getElementById('next_users').disabled = false;
        if(items.curPage==0){
            document.getElementById('prev_users').disabled = true;
        }else document.getElementById('prev_users').disabled = false;
        ...
        if(items.pageCount>1){
            ...
            document.getElementById('next_users').style.visibility="visible";
            document.getElementById('prev_users').style.visibility="visible"; 
            document.getElementById('showPages').style.visibility="visible";   
        }else {
            document.getElementById('showPages').style.visibility="hidden";
            document.getElementById('next_users').style.visibility="hidden";
            document.getElementById('prev_users').style.visibility="hidden";
        }
    })
